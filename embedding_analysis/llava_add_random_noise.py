...
def parse_args():
    ...
    parser.add_argument("--model_path", type=str, help="specify the gpu to load the model.")
...
my_attacker = visual_attacker.Attacker(args, model, tokenizer, [''], device=model.device, image_processor=image_processor)
...
with torch.no_grad():
    for image_file in tqdm(attacked_image_files):
        ...
        #raw_img = image_processor(raw_img).unsqueeze(0).to(model.device)
