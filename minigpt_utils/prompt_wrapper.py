...
minigpt4_chatbot_prompt_ignore_img = "Give the following image: <Img>ImageContent</Img>. \
You will be able to see the image once I provide it to you. \
Ignore the image and answer my questions.###Human: <Img><ImageHere></Img> \
%s###Assistant:"
...
class PreferencePrompt:
    ...
    def __init__(
            self, model, text_formats=None, text_prompts=None, img_prompts=None, device='cuda:0',
            max_new_tokens=300, max_length=2000
    ):
        ...
        self.text_embs = self.generate_text_embedding(self.text_formats, self.text_prompts)
        self.img_embs = self.generate_img_embedding(self.img_prompts)
        self.update_context_embs()
    ...
    def update_text_prompt(self, text_formats, text_prompts):
        self.text_formats = text_formats
        self.text_prompts = text_prompts
        self.text_embs = self.generate_text_embedding(self.text_formats, self.text_prompts)
        self.update_context_embs()
    ...
    def generate_text_embedding(self, text_formats, text_prompts):

        if text_formats is None:
            return []

        text_embs = []
        for text_format, prompt in zip(text_formats, text_prompts):  # for each prompt within a batch
            prompt_segs = (text_format % prompt).split('<ImageHere>')  # each <ImageHere> corresponds to one image
            seg_tokens = [
                self.model.llama_tokenizer(
                    seg, return_tensors="pt", add_special_tokens=i == 0).to(self.device).input_ids
                # only add bos to the first seg
